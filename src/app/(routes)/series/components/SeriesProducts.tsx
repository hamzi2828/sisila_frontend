...
export default function SeriesProducts({
  section,
}: {
  section: Section;
}) {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const response = await publicProductService.getProductsByCollection('series', id, 8);
        setProducts(response.data);
      } catch (err) {
        console.error('Error fetching series products:', err);
      ...
      >
        ...
        <div className="relative z-10 p-6 md:p-8 lg:p-10">
          ...
          {!isLoading && !error && products.length === 0 && (
            <div className="text-center py-12">
              <p className="text-stone-500">No products available for this series yet.</p>
            </div>
          )}
          ...
          {!isLoading && !error && products.length > 0 && (
            <>
              ...
              <div className="mt-8 text-center">
                <Link
                  href={`/shop?collection=series&collectionId=${id}`}
